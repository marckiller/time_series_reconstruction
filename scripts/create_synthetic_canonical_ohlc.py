...
def parse_args():
    ...
    parser.add_argument(
        "--trajectory-regimes",
        nargs="+",
        choices=["gbm", "stochastic_vol", "jumps", "mixed"],
        default=["gbm"],
    )
    parser.add_argument("--vol-block-minutes", type=int, default=240)
    parser.add_argument("--vol-min", type=float, default=0.5)
    parser.add_argument("--vol-max", type=float, default=2.5)
    parser.add_argument("--jump-intensity-per-hour", type=float, default=0.08)
    parser.add_argument("--jump-scale", type=float, default=6.0)
    parser.add_argument("--format", choices=["parquet", "csv"], default="parquet")
    return parser.parse_args()


def piecewise_volatility(
    n: int,
    block_minutes: int,
    vol_min: float,
    vol_max: float,
    ...
    n_blocks = int(np.ceil(n / block_minutes))
    block_vols = rng.lognormal(mean=0.0, sigma=0.45, size=n_blocks)
    block_vols = np.clip(block_vols, vol_min, vol_max)
    return np.repeat(block_vols, block_minutes)[:n]
...
def generate_jump_returns(
    ...
    if intensity_per_hour <= 0:
        return np.zeros(n)
    jump_probability = intensity_per_hour / 60.0
    jump_mask = rng.random(n) < jump_probability
    jumps = rng.normal(0.0, jump_scale * sigma, size=n)
    return jump_mask * jumps
...
def generate_correlated_log_returns(
    ...
    index_vol = np.ones(n)
    target_vol = np.ones(n)
    if regime in {"stochastic_vol", "mixed"}:
        common_vol = piecewise_volatility(n, vol_block_minutes, vol_min, vol_max, rng)
        target_extra_vol = piecewise_volatility(n, vol_block_minutes, vol_min, vol_max, rng)
        index_vol = common_vol
        target_vol = 0.65 * common_vol + 0.35 * target_extra_vol

    index_returns = index_sigma * index_vol * z_index
    target_returns = target_sigma * target_vol * z_target

    if regime in {"jumps", "mixed"}:
        common_jumps = generate_jump_returns(
            n,
            sigma=index_sigma,
            intensity_per_hour=jump_intensity_per_hour,
            jump_scale=jump_scale,
            rng=rng,
        )
        index_jumps = common_jumps + generate_jump_returns(
            n,
            sigma=index_sigma,
            intensity_per_hour=jump_intensity_per_hour * 0.5,
            jump_scale=jump_scale,
            rng=rng,
        )
        target_jumps = corr * common_jumps + generate_jump_returns(
            n,
            sigma=target_sigma,
            intensity_per_hour=jump_intensity_per_hour,
            jump_scale=jump_scale,
        ...
        index_returns = index_returns + index_jumps
        target_returns = target_returns + target_jumps

    return index_returns, target_returns
...
def main():
    ...
    summary_rows = []
    pair_id = 0
    for regime in args.trajectory_regimes:
        for target_sigma in args.target_sigmas:
            for corr in args.correlations:
                ...
                index_returns, target_returns = generate_correlated_log_returns(
                    ...
                    rng=rng,
                    regime=regime,
                    vol_block_minutes=args.vol_block_minutes,
                    vol_min=args.vol_min,
                    vol_max=args.vol_max,
                    jump_intensity_per_hour=args.jump_intensity_per_hour,
                ...
                print(
                    f"{target_ticker}: regime={regime}, corr={corr:.2f}, "
                    f"realized={realized_corr:.3f}, target_sigma={target_sigma:g}"
                )
