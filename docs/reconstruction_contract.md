...
This document defines the intended reconstruction task for this project. It is the reference point for data preparation, model inputs, training, evaluation, and API behavior.
...
- 5-minute target data may populate slots `0, 5, 10, ..., 55`,
- 10-minute target data may populate slots `0, 10, 20, ..., 50`,
- 15-minute target data may populate slots `0, 15, 30, 45`,
- irregular data may populate any subset of slots,
...
- `1.0`: this slot is observed and may be used by the model,
- `0.0`: this slot is missing or intentionally hidden.

Unobserved target slots are represented as `NaN` during data preparation. They may be converted to `0.0` only after the mask has been created. The zero value itself must never be interpreted as an observed price or normalized price unless the mask for that slot is `1.0`.

For example, sparse target observations may be encoded after mask construction as:
...
The same contract applies during training, evaluation, and inference. A model should not need to know whether a sparse vector came from 5-minute bars, 15-minute bars, irregular missingness, or an API request.

## Known Inputs

For each target-stock interval, the following information may be used:
...
Known target-stock observations should be preserved in the output unless an explicit post-processing or denoising mode says otherwise.
...
- values may be outside `[0, 1]` during raw model inference, but evaluation and production post-processing should report whether this happened.
...
Missing numeric values should be represented as `NaN` in data preparation and converted to zero only after masks are created.
...
For offline historical enhancement, rolling relationship features may include the current completed interval.
...
- `y` is the target-stock point-price series used for supervision and may also be sparse,
- `y_mask` marks target-stock slots where the true value is known,
- loss should be computed only where `y_mask == 1`,
- reconstruction quality should be measured primarily where `y_mask == 1` and `X_ts_mask == 0`.
...
The train/validation/test split should be time-aware. Random row-level splitting is allowed only for quick smoke tests and must not be reported as final evidence.
...
Neural models should be compared against simple baselines before claims are made.
...
Pointwise metrics may be reported:

- MSE,
- MAE,
- R2.

They are not sufficient on their own. Reconstruction should also report:
...
- output should be interpreted as a plausible imputation,
- low pointwise error does not guarantee correct hidden price dynamics,
- the project should not be described as recovering uniquely determined hidden data,
